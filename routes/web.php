...
Route::prefix('inventory/{system}')->group(function () {
    ...
    Route::get('/inventory', function ($system) {
        $categoryMap = ['usher' => 1, 'usherette' => 2, 'wehlo' => 3, 'hoclomac' => 4, 'all' => 5];
        return Inertia::render('Inventory/Inventory', [
            'mainCategoryId' => $categoryMap[$system] ?? 1,
            'system' => $system,
        ]);
    })->name('inventory.home')->where('system', 'usher|usherette|wehlo|hoclomac|all');
    // Master list - shows all boxes
    Route::get('/master-list', [MasterListController::class, 'index'])
        ->name('inventory.master-list')
        ->where('system', 'usher|usherette|wehlo|hoclomac|all');
    ...
    Route::get('/master-list/box/{boxId}/item/{subcategoryId}', [MasterListController::class, 'showSerials'])
        ...
        ->where([
            'system' => 'usher|usherette|wehlo|hoclomac|all',
            'boxId' => '[0-9]+',
            'subcategoryId' => '[0-9]+'
        ]);
    ...
    })->name('inventory.stock-in')->where('system', 'usher|usherette|wehlo|hoclomac|all');
    ...
    })->name('inventory.stock-out')->where('system', 'usher|usherette|wehlo|hoclomac|all');
    ...
    })->name('inventory.in-use')->where('system', 'usher|usherette|wehlo|hoclomac|all');
    ...
    })->name('inventory.damaged')->where('system', 'usher|usherette|wehlo|hoclomac|all');
    ...
    })->name('inventory.purchase-order')->where('system', 'usher|usherette|wehlo|hoclomac|all');
    ...
    })->name('inventory.purchase-request')->where('system', 'usher|usherette|wehlo|hoclomac|all');
    ...
    })->name('inventory.deployment')->where('system', 'usher|usherette|wehlo|hoclomac|all');
});