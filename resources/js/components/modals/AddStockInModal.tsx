...
export const AddStockInModal: React.FC<AddStockInModalProps> = ({
  ...
  const validateForm = () => {
    ...
    // Serial numbers are optional — blank entries are auto-generated by the backend
    setErrors(newErrors);
  ...
  const handleAddItem = () => {
    ...
    // Send all qty slots — blanks are auto-generated by the backend
    const serialsToSend = Array.from({ length: qty }, (_, i) => serialNumbers[i] ?? '');
    ...
    const newItem: StockInItem = {
      ...
      quantity: qty,
      remarks,
      serialGroups: [{ serialNumbers: serialsToSend, supplierId: selectedSupplier!.id, supplierName: selectedSupplier!.name }],
    };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                    Serial Identifiers
                    <span className="ml-1.5 text-[9px] font-bold text-blue-400 normal-case tracking-normal">(optional — leave blank to auto-generate)</span>
                  </label>
                  <div className="border-2 rounded-2xl p-2 bg-slate-50/50 dark:bg-slate-950/10 max-h-[140px] overflow-y-auto space-y-2 custom-scrollbar border-slate-100 dark:border-slate-800">
                    {serialNumbers.slice(0, parseInt(quantity || '0')).map((sn, idx) => (
                      <input key={idx} value={sn} onChange={(e) => handleSerialChange(idx, e.target.value)} placeholder={`SN #${idx + 1} — leave blank to auto-generate`} className="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none placeholder:font-normal placeholder:text-slate-300 dark:placeholder:text-slate-600" />
                    ))}
